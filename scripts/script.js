...
function showTime() {
  const date = new Date();
  timeDisplay.textContent = date.toLocaleTimeString('nl-NL');
  showDate();
  getTimeOfDay();
  showGreeting();
...
function getGithubImageURL() {
  ...
  return `https://raw.githubusercontent.com/irliss/stage1-tasks/assets/images/${timeOfDay}/${bgNum}.jpg`;
}

async function getUnsplashImageURL() {
  const id = '-Aduh2uS_yEUqTwti3BcSyabBbBlbo3DfFt16PBE9EI';
...
async function getFlickrImageURL() {
  const id = '042467caf694c31e6368e0a9b8edebd6';
...
function onlyAlphabets() {
  ...
  return regex.test(tagInput.value);
}
